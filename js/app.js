...
function catalogBooks(book) {
  ...
  const button = UpdaterDom.button(
    "Comprar",
    "btn btn-success",
    book.id,
    book.stock
...
function alert(message, type) {
  var alertPlaceholder = document.getElementById("modal-succes");
  var wrapper = document.createElement("div");
  wrapper.setAttribute("id", "abc");
  wrapper.innerHTML =
...
function finalizarCompra() {
  if (cart.length === 0) {
    alert("No hay productos en el carro", "danger");
  } else {
    alert("Los datos seran enviados a su email", "success");
    cart = [];
    renderizarCarrito();
    calcularTotal();
    $("#cart").html(` Mi carrito ${cart.length}`);
